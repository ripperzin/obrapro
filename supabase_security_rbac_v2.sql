-- ==============================================================================
-- MIGRATION: SEGURANÇA RBAC (V2 - CORRIGIDO)
-- DATA: 25/01/2025
...
returns trigger as $$
begin
  -- Tenta inserir o usuário logado (auth.uid()) como owner
...
-- ==============================================================================
-- 6. MIGRAÇÃO DE DADOS (USANDO LOGS)
-- ==============================================================================

-- A. Inserir Profiles para todos os usuários existentes
insert into public.profiles (id, email)
select id, email from auth.users
on conflict (id) do nothing;

-- B. Descobrir donos via tabela de LOGS (onde action = 'Criação')
-- Tenta converter user_id (string) para uuid. Se falhar, ignora.
insert into public.project_members (project_id, user_id, role)
select 
    l.project_id::uuid, 
    l.user_id::uuid, 
...
where l.action = 'Criação'
  and l.user_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' -- Regex Check UUID
on conflict (project_id, user_id) do nothing;

-- C. FAILSAFE: Se sobrar projeto sem dono, atribui a VOCÊ (quem está rodando o script)
insert into public.project_members (project_id, user_id, role)
